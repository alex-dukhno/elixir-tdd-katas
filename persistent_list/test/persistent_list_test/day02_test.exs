defmodule PersistentListTest.Day02 do
  use ExUnit.Case

  import PersistentList.Day02
  ...
  test "prepend an item to a list", do:
    assert empty()
           |> prepend(1)
           |> to_string == "[1]"

  test "prepend many items to a list", do:
    assert empty()
           |> prepend(1)
           |> prepend(2)
           |> prepend(3)
  ...
  test "drop many items from a nonempty list", do:
    assert from_to(1, 6)
           |> drop(3)
           |> to_string == "[3, 2, 1]"

  test "drop 0 item from nonempty list", do:
    assert from_to(1, 3)
           |> drop(0)
           |> to_string == "[3, 2, 1]"
  ...
  test "take items from a nonempty list", do:
    assert from_to(1, 3)
           |> take(2)
           |> to_string == "[3, 2]"

  test "take items from an empty list by predicate", do:
    assert empty()
