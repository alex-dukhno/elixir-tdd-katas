defmodule PersistentListTest.Day04 do
  use ExUnit.Case

  import PersistentList.Day04
  ...
  test "concat two empty lists", do:
    assert empty()
           |> concat(empty())
           |> to_string == "[]"

  test "concat empty and nonempty list", do:
    assert empty()
           |> concat(from_to(1, 3))
           |> to_string == "[3, 2, 1]"

  test "concat two nonempty lists", do:
    assert from_to(1, 3)
           |> concat(from_to(4, 6))
           |> to_string == "[3, 2, 1, 6, 5, 4]"
