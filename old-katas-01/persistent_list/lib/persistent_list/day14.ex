defmodule PersistentList.Day14 do
  alias PersistentList.Day14, as: List
  ...
  defimpl String.Chars, for: List do
    ...
    def stringify(%List{head: nil, tail: nil}), do: ""
    def stringify(
          %List{
            head: head,
            tail: %List{
    ...
    def stringify(%List{head: head, tail: tail}), do: "#{head}, " <> stringify(tail)
  end
  ...
  def prepend(%List{head: nil, tail: nil} = empty, item), do:
    empty
    |> append(item)
  def prepend(%List{head: head, tail: tail}, item), do:
    tail
    |> prepend(item)
    |> append(head)

  def concat(%List{head: nil, tail: nil}, other), do: other
  def concat(%List{head: head, tail: tail}, other), do:
    tail
    |> concat(other)
    |> append(head)
  ...
  def drop(%List{tail: tail}, num), do:
    tail
    |> drop(num - 1)
